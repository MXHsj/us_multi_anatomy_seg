from __future__ import annotations

import platform
import subprocess
import sys


DEVICE = "cpu" if platform.system() == "Darwin" else "cuda:0"
BENCHMARK_SCRIPT = "benchmarks/medical_sam3_inference.py"
DATASET_ROOT = "datasets/USSC"
OUTPUT_DIR = "results/medicalsam3_text_prompt_ussc"

# Default concept label per target comes from per-sample metadata plus
# datasets/label_text.py, matching other multi-class decoders.
DEFAULT_ARGS = [
    "--dataset",
    "ussc",
    "--dataset-root",
    DATASET_ROOT,
    "--max-samples",
    "all",
    "--device",
    DEVICE,
    "--save-vis",
    "20",
    "--output-dir",
    OUTPUT_DIR,
]


def main() -> None:
    cmd = [sys.executable, BENCHMARK_SCRIPT, *DEFAULT_ARGS, *sys.argv[1:]]
    print(f"Starting Medical SAM3 text benchmark: {' '.join(cmd)}", flush=True)
    subprocess.run(cmd, check=True)


if __name__ == "__main__":
    main()
