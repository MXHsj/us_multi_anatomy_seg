#!/usr/bin/env python3
"""Run SAMUS (GT box prompts) on selected datasets sequentially.
...
Any extra CLI args are forwarded to every benchmark, e.g.:
    ...
    python work_dir/SAMUS/run_samus.py --max-samples 50 --device cpu
"""
...
# Repo root is two levels up from this file (work_dir/SAMUS/..).
...
def main() -> None:
    ...
    print(f"Running SAMUS on {len(selected)} dataset(s): {', '.join(selected)}")

    failures: list[str] = []
    for name in selected:
        script = REPO_ROOT / "benchmarks" / f"test_samus_gt_bbox_{name}.py"
