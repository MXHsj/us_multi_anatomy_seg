...
def create_yolo_dataset():
    ...
    image_paths = sorted(glob.glob(os.path.join(DATA_SOURCE_DIR, 'PHANTOM_*', 'US_*.jpg')))
    ...
    for split, paths in [('train', train_paths), ('val', val_paths)]:
        ...
        for i, img_path in enumerate(paths):
            # Get image details
            phantom_dir = os.path.basename(os.path.dirname(img_path))
            original_img_filename = os.path.basename(img_path)
            
            # Create a new unique filename
            new_img_filename = f"{phantom_dir}_{original_img_filename}"
